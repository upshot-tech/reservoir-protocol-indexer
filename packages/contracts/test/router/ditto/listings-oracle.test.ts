...
describe("DittoModule", () => {
  ...
  beforeEach(async () => {
    ...
    getDittoContracts().then((contracts) => {
      nft = contracts.nft;
  ...
  it("Upshot oracle test", async () => {
    ...
    const authenticatorSigner: SignerWithAddress = await ethers.getImpersonatedSigner(
    ...
    const templateInitData: any = ethers.utils.defaultAbiCoder.encode(
      ...
      [Sdk.Ditto.Addresses.UpshotOracle[5], "1", "10000000000000000000"]
    );
    ...
    const txn06 = await dittoPoolFactory
      .connect(deployer)
    ...
    let messageHash = ethers.utils.solidityKeccak256(
      ["uint256", "uint256", "address", "uint256", "address", "uint256", "uint96", "uint96"],
      [chainId, nonce, nft.address, tokenId04, token.address, price, timestamp, expiration]
    );
