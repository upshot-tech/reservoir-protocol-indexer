...
import { DittoswapPoolKind, DittoswapV2PoolKind } from "@/models/dittoswap-pools";
...
export const save = async (orderInfos: OrderInfo[]): Promise<SaveResult[]> => {
  ...
  const handleOrder = async ({ orderParams }: OrderInfo) => {
    try {
      ...
      // Handle: fees
      // TODO: How do we do fees?  recipient is SudoGovernor
      const feeBps = 50;
      const feeBreakdown: {
        kind: string;
        recipient: string;
      ...
      try {
        if ([DittoswapV2PoolKind.TOKEN, DittoswapV2PoolKind.TRADE].includes(pool.poolKind)) {
          ...
          await Promise.all(
            _.range(0, POOL_ORDERS_MAX_PRICE_POINTS_COUNT).map(async (index) => {
              try {
                // TODO use the method from the poolContract TBD above
          ...
          if (prices.length) {
            ...
            if (!orderResult) {
              ...
              // TODO: change into getting the fixed limit
      ...
      try {
        if ([DittoswapPoolKind.NFT, DittoswapPoolKind.TRADE].includes(pool.poolKind)) {
          let tmpPriceList: (BigNumber | undefined)[] = Array.from(
            { length: POOL_ORDERS_MAX_PRICE_POINTS_COUNT },
            () => undefined
          );
            ...
            poolOwnedTokenIds.map(({ tokenId, amount }) =>
              limit(async () => {
                try {
                  ...
                  // Handle: core sdk order
                  const sdkOrder: Sdk.Dittoswap.Order = new Sdk.DittowapV2.Order(config.chainId, {
                    pair: orderParams.pool,
                    ...
                    await idb.none(
                      `
                        UPDATE orders SET
                          ...
                          valid_between = tstzrange(date_trunc('seconds', to_timestamp(${orderParams.txTimestamp})), 'Infinity', '[]'),
                          -- TODO limited expiration , not infinity
                          expiration = 'Infinity',
                          updated_at = now(),
                          raw_data = $/rawData:json/,
                          missing_royalties = $/missingRoyalties:json/,
